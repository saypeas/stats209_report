...
cocs = read.csv("/Users/viyan/Downloads/stats209/output/tables/sedd_cocs_in_bandwidth.csv") |>
...
acs3 = left_join(acs2, sedd, by = c("coc.number.new", "year"))
...
sample <- sample %>%
  mutate(
    ed_per_10k_homeless = (health_ed+1 / ov.homeless) * 10000
  )

# --- Calculate change in ed_per_10k_homeless from 2015 to 2016 ---
# First create a dataset with just 2015 values
ed_2015 <- sample %>%
  filter(year == 2015) %>%
  select(coc.number.new, ed_per_10k_homeless) %>%
  rename(ed_per_10k_2015 = ed_per_10k_homeless)

# Then create a dataset with just 2016 values
ed_2016 <- sample %>%
  filter(year == 2016) %>%
  select(coc.number.new, ed_per_10k_homeless) %>%
  rename(ed_per_10k_2016 = ed_per_10k_homeless)

# Join them together
ed_change <- full_join(ed_2015, ed_2016, by = "coc.number.new") %>%
  mutate(
    ed_change_2015_2016 = (ed_per_10k_2016 - ed_per_10k_2015)/(ed_per_10k_2015)
  )

# Add this back to the main sample
sample <- left_join(sample,
                    select(ed_change, coc.number.new, ed_change_2015_2016),
                    by = "coc.number.new")
...
panel_c_data <- sample %>%
  ...
  summarize(
    ...
    ed_change_2015_2016 = mean(ed_change_2015_2016, na.rm = TRUE),
    .groups = "drop"
  )
...
summary_stats <- panel_c_data %>%
  group_by(d.esg.extra) %>%
  summarize(
    Mean = mean(ed_change_2015_2016, na.rm = TRUE),
    SD = sd(ed_change_2015_2016, na.rm = TRUE),
    .groups = "drop"
  )

ttest_result <- perform_ttest(panel_c_data, "ed_change_2015_2016")

panel_c_results <- data.frame(
  Variable = "ed_change_2015_2016",
  Mean_Control = summary_stats$Mean[summary_stats$d.esg.extra == 0],
  Mean_Treatment = summary_stats$Mean[summary_stats$d.esg.extra == 1],
  SD_Control = summary_stats$SD[summary_stats$d.esg.extra == 0],
  SD_Treatment = summary_stats$SD[summary_stats$d.esg.extra == 1],
...
# Write to CSV
write.csv(combined_table, "~/Downloads/stats209/output/tables/balance_table_sedd_with_ttests_and_se.csv", row.names = FALSE)
